...
class TestPostPayments:
    ...
    def test_post_payment_without_transaction_id(self, user_headers, sample_payment_data_no_transaction_id):
        """Trying to add payment without transaction field"""
        res = requests.post(url=f"{BASE_URL}payments", json=sample_payment_data_no_transaction_id, headers=user_headers)
        assert res.status_code in [400, 422]
    
    def test_post_payment_without_validation_hash(self, user_headers, sample_payment_data_no_validation_hash):
        """Trying to add payment without validation field"""
        res = requests.post(url=f"{BASE_URL}payments", json=sample_payment_data_no_validation_hash, headers=user_headers)
        assert res.status_code in [400, 422]

    def test_post_payment_amount_is_negative(self, user_headers, sample_payment_data_amount_negative):
        """Trying to add payment where amount is negative"""
        res = requests.post(url=f"{BASE_URL}payments", json=sample_payment_data_amount_negative, headers= user_headers)
        assert res.status_code in [400, 422]
    
    def test_post_authenticated_payment_with_all_required_fields(self, user_headers, sample_payment_data):
        """Trying to add payment with authenticated user"""
        res = requests.post(url=f"{BASE_URL}payments", json=sample_payment_data, headers= user_headers)
        assert res.status_code == 201

    def test_post_unauthenticated_payment_with_all_required_fields(self, user_headers, sample_payment_data):
        """Trying to add payment with unauthenticated user"""
        res = requests.post(url=f"{BASE_URL}payments", json=sample_payment_data, headers={"Authorization": "false_auth"})
        assert res.status_code == 401
    
    def test_post_payment_refund_user_with_all_required_fields(self, user_headers, sample_payment_data):
        """Trying to add a refund with authenticated user"""
        res = requests.post(url=f"{BASE_URL}refund", json=sample_payment_data, headers=user_headers)
        assert res.status_code == 403
    
    def test_post_payment_refund_admin_with_all_required_fields(self, admin_headers, sample_payment_data):
        """Trying to add a refund with authenticated admin"""
        res = requests.post(url=f"{BASE_URL}refund", json=sample_payment_data, headers=admin_headers)
...
class TestPutPayments:
    ...
    def test_update_payment_missing_validation_hash(self, user_headers, sample_payment_data, sample_payment_data_no_validation_hash):
    ...
    def test_update_other_user_payment(self, user_headers, admin_headers, sample_payment_data):
    ...
    def test_admin_can_update_any_payment(self, user_headers, admin_headers, sample_payment_data):
