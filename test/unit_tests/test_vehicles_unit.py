...
MOCK_ADMIN = {"username": "testuser", "role": "USER"}
...
class TestCreateVehicle:
    @path("routers.vehicles_router.get_session")
    @patch("routers.vehicles_router.load_vehicle_data_from_db")
    @patch("routers.vehicles_router.save_vehicle_data_to_db")
    @patch("uuid.uuid4")
...
@patch("routers.vehicles_router.get_session")
...
@patch("routers.vehicles_router.get_session")
@path("routers.vehicles_router.load_vehicle_data_from_db")
...
class TestGetVehicles:
    @patch("routers.vehicles_router.get_session")
    @patch("routers.vehicles_router.get_vehicle_data_by_user")
...
class TestUpdateVehicle:
    @patch("routers.vehicles_router.get_session")
    @patch("routers.vehicles_router.find_vehicle_by_license_plate")
    @patch("routers.vehicles_router.update_existing_vehicle_in_db")
...
class TestDeleteVehicle:
    @patch("routers.vehicles_router.get_session")
    @patch("routers.vehicles_router.find_vehicle_by_license_plate")
    @patch("routers.vehicles_router.delete_vehicle_from_db")
    ...
    mock_session.return_value = MOCK_USER
    mock_find.return_value = MOCK_VEHICLE_2.copy()
    response = client.delete("/vehicles/")
    ...
    @patch("routers.vehicles_router.find_vehicle_by_license_plate")
    def test_delete_other_user_vehicle_forbidden(self, mock_find, mock_session):
        """Test non-admin cannot delete another user's vehicle"""
        mock_session.return_value = MOCK_USER
        mock_find.return_value = MOCK_VEHICLE_2.copy()
    ...
    def test_admin_delete_other_user_vehicle(self, mock_delete, mock_find, mock_session):
        """Test admin can delete another user's vehicle"""
        mock_session.return_value = MOCK_ADMIN
        mock_find.return_value = MOCK_VEHICLE.copy()
        mock_delete.return_value = True
...
class TestGetVehicleReservations:
    """Unit tests for GET /vehicles/{license_plate}/reservations"""

    @patch("routers.vehicles_router.get_session")
    @patch("routers.vehicles_router.find_vehicle_by_license_plate")
    def test_get_own_vehicle_reservations(self, mock_find, mock_session):
        """Test getting reservations for user's own vehicle"""
        mock_session.return_value = MOCK_USER
        mock_find.return_value = MOCK_VEHICLE.copy()
    ...
    def test_get_other_user_vehicle_reservations_forbidden(self, mock_find, mock_session):
        """Test non-admin cannot get another user's vehicle reservations"""
        mock_session.return_value = MOCK_USER
        mock_find.return_value = MOCK_VEHICLE_2.copy()
    ...
    def test_get_reservations_nonexistent_vehicle(self, mock_load, mock_session):
        """Test getting reservations for nonexistent vehicle"""
        mock_session.return_value = MOCK_USER
        mock_load.return_value = []
...
class TestGetVehicleHistory:
    """Unit tests for GET /vehicles/{license_plate}/history"""

    @patch("routers.vehicles_router.get_session")
    @patch("routers.vehicles_router.find_vehicle_by_license_plate")
    @patch("routers.vehicles_router.load_parking_lot_data")
    @patch("routers.vehicles_router.load_parking_session_data")
    def test_get_own_vehicle_history(self, mock_load_sessions, mock_load_lots, mock_find, mock_session):
        """Test getting history for user's own vehicle"""
        mock_session.return_value = MOCK_USER
        mock_find.return_value = MOCK_VEHICLE.copy()
    ...
    def test_get_other_user_vehicle_history_forbidden(self, mock_find, mock_session):
        """Test non-admin cannot get another user's vehicle history"""
        mock_session.return_value = MOCK_USER
        mock_find.return_value = MOCK_VEHICLE_2.copy()
    ...
    def test_get_history_nonexistent_vehicle(self, mock_load, mock_session):
        """Test getting history for nonexistent vehicle"""
        mock_session.return_value = MOCK_USER
        mock_load.return_value = []
    ...
    def test_get_vehicle_history_admin_access(
        ...
        """Test admin can get any user's vehicle history"""
        mock_session.return_value = MOCK_ADMIN
        mock_find.return_value = MOCK_VEHICLE.copy()
...
class TestNormalizePlate:
    ...
    def test_normalize_plate_empty(self):
        """Test normalizing empty plate"""
        from routers.vehicles_router import normalize_plate

        assert normalize_plate("") == ""

    def test_normalize_plate_none(self):
        """Test normalizing None plate"""
        from routers.vehicles_router import normalize_plate
